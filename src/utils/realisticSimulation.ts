...
// Calculate precise heading based on previous, current, and next positions
export const calculatePreciseHeading = (
  truck: Truck,
  nextPosition: Position
): number => {
  const current = truck.currentPosition;
  const previous = truck.previousPosition;

  // If we have previous position, use it for more accurate heading
  if (previous) {
    // Calculate heading from previous to current position (actual movement)
    const actualHeading = calculateBearing(previous, current);

    // Calculate heading from current to next position (intended direction)
    const intendedHeading = calculateBearing(current, nextPosition);

    // Blend actual and intended headings for smooth transition
    const headingDiff = ((intendedHeading - actualHeading + 540) % 360) - 180;
    const smoothedHeading = (actualHeading + headingDiff * 0.3 + 360) % 360;

    return smoothedHeading;
  } else {
    // Fallback to simple bearing calculation
    return calculateBearing(current, nextPosition);
  }
};

// Simulate realistic truck movement with precise heading
export const simulateRealisticMovement = (truck: Truck): Partial<Truck> => {
  ...
  else if (truck.route.includes("Highway 605")) highway = "Highway_40"; // Use default for bridge routes
  ...
  if (!nearestWaypoint || !route) {
    ...
    if (distance < 1) {
      // Less than 1km, mark as completed
      return {
        status: "completed",
        speed: 0,
    ...
    const newLat =
      currentPos.lat + (destination.lat - currentPos.lat) * moveRatio;
    const newLng =
      currentPos.lng + (destination.lng - currentPos.lng) * moveRatio;
    const newPosition = {
      lat: newLat,
      lng: newLng,
    ...
    // Calculate precise heading
    const heading = calculatePreciseHeading(truck, newPosition);
  ...
  if (distanceToWaypoint < moveDistance) {
    ...
    // Calculate precise heading using previous position
    const heading = calculatePreciseHeading(truck, newPosition);
    ...
    const newLat =
      currentPos.lat + (nextWaypoint.lat - currentPos.lat) * moveRatio;
    const newLng =
      currentPos.lng + (nextWaypoint.lng - currentPos.lng) * moveRatio;
...
export const generateRandomEvent = (truck: Truck): any[] => {
  ...
  if (random < 0.05) {
    ...
    events.push({
      ...
      type: "speed",
      message: `Speed: ${Math.round(truck.speed)} km/h on ${
        truck.route.split(" - ")[0]
      }`,
